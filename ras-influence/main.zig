...
pub fn main() !void {
    ...
    if (args.len != 5) {
        std.debug.print("Usage: {s} <used_core> <try_count> <call_depth> <call_amount>\n", .{args[0]});
        return error.InvalidArguments;
    }
    const used_core = try std.fmt.parseInt(i32, args[1], 10);
    const try_count = try std.fmt.parseInt(usize, args[2], 10);
    const call_depth = try std.fmt.parseInt(usize, args[3], 10);
